...
bool http_message(struct stream stream)
{
  ...
  // this is optional. it contains the content length or other fields?
...
static bool field_line(struct stream stream)
{
  ...
  while ((next.type == VCHAR || next.type == DIGIT || next.type == ALPHA || next.type == SP) && stream.available())
  {
...
static bool field_value(struct stream stream)
{
  ...
  if (next.type == VCHAR || next.type == SP || next.type == HTAB)
  {
    readToken(stream);
    return true;
  }
...
static bool http_token(struct stream stream)
{
  ...
  for (int i = 0; i < 7; ++i)
  {
    if (!tchar(stream, result, sizeof(result)))
    {
      ...
      // printf("tchar: token type: '%d'\n", peekToken(stream).type);
      // printf("tchar: token value: '%s'\n", peekToken(stream).value);
      break;
    }
  }
  printf("HTTP_TOKEN: '%s'\n", result);
...
static bool message_body(struct stream stream)
{
  ...
  while ((next.type == VCHAR || next.type == DIGIT || next.type == ALPHA || next.type == SP) && stream.available())
  {
    ...
    catValue(current.value, result, sizeof(result));
...
static bool origin_form(struct stream stream)
{
  ...
  while (next.type == VCHAR || next.type == DIGIT || next.type == ALPHA || next.type == SP)
  {
  ...
  printf("origin_form: expected type: '%d', '%d', '%d'\n", VCHAR, DIGIT, ALPHA);
