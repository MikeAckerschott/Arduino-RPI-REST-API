...
bool hasTokenType(struct token token, enum tokentype type)
{
  ...
  //printf("hasTokenType: token type mismatch\n");
  return false;
}
                   ...
                   const char *value)
{
  ...
  {
    //printf("hasTokenValue: token type mismatch\n");
    //printf("hasTokenValue: token type: '%d'\n", token.type);
    //printf("hasTokenValue: expected type: '%d'\n", type);
    //printf("hasTokenValue: token value: '%s'\n", token.value);
    //printf("hasTokenValue: expected value: '%s'\n", value);
    return false;
  }

  if (strlen(value) > TOKEN_LENGTH)
  {
    //printf("hasTokenValue: value too long\n");
    return false;
  }


  if (strncmp(token.value, value, TOKEN_LENGTH) == 0){
    return true;
  }
  //printf("hasTokenValue: token value: '%s'\n", token.value);
  //printf("hasTokenValue: expected value: '%s'\n", value);
  //printf("hasTokenValue: token value length: %d\n",strlen(token.value));
  //printf("hasTokenValue: expected value length: %d\n",strlen(value));
}
